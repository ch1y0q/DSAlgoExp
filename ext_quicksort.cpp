...
#define PRINT_TIME_SO_FAR std::cout<< "Elasped time: " \
              << float(clock() - begin_time) / CLOCKS_PER_SEC * 1000 \
              << " msec." << std::endl;
#define PRINT_SEPARATOR_START std::cout << "-------------------------------------------------------\n";
#define PRINT_SEPARATOR_END std::cout << "-------------------------------------------------------\n\n\n";
...
void ext_qsort(TreeNode<std::string> *cur_node)
{
    ...
    MIDDLE_GROUP_MEM = TOTAL_MEM-INPUT_BUFFER_MEM-SMALL_GROUP_MEM-LARGE_GROUP_MEM;
    ...
    /* init reader */
    ext_qsort_t<T> manager(INPUT_BUFFER_SIZE, SMALL_GROUP_SIZE,LARGE_GROUP_SIZE,MIDDLE_GROUP_SIZE);
    ...
    while (!input.eof()) {
        /* feed input_buffer */
        while(manager.input_buffer.size() < INPUT_BUFFER_SIZE && !input.eof()){ 
            T cur_data;
        ...
        /* digest fed data */
        while(manager.middle_group.size()<MIDDLE_GROUP_SIZE && !manager.input_buffer.empty()){
        ...
        while(!manager.input_buffer.empty()){
            T cur_data = manager.input_buffer.back();
            manager.input_buffer.pop_back();
            T largest = *std::max_element(manager.middle_group.begin(), manager.middle_group.end());
            T smallest = *std::min_element(manager.middle_group.begin(), manager.middle_group.end());
            if(cur_data > largest){
                ...
                auto pos = std::max_element(manager.middle_group.begin(), manager.middle_group.end());
            ...
            if(manager.large_group.size()>=LARGE_GROUP_SIZE){
                ...
                // write to disk!
                std::fstream fs(save_name, std::ios::out | std::ios::binary | std::ios::app);
                if (!fs) {
...
std::cout<<std::endl;
    ...
    {
        std::fstream fs(cur_node->data, std::ios::out | std::ios::binary);   // no std::ios::app
        if (!fs) {
